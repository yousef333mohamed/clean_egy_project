...
PROMPT_KEYS = frozenset(
    {"rag_system", "rag_answer", "query_rewrite", "analytics_router", "analytics_answer", "hybrid_answer", "decision_router", "candidate_actions", "decision_explanation"}
)
SEMVER_PATTERN = re.compile(r"^\d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?$")
CREDENTIAL_PATTERN = re.compile(
    r"(?i)(?:api[_-]?key|password|secret|authorization)\s*[:=]\s*(?!\{|\[|<|REDACTED)[\"']?[A-Za-z0-9_./+\-=]{8,}"
)
