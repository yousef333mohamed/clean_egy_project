...
router = APIRouter(prefix="/prompts", tags=["prompt administration"])
...
async def list_versions(prompt_key: str, session: DatabaseSession, settings: AppSettings):
    ...
    return list((await session.execute(select(PromptVersion).where(PromptVersion.prompt_key == prompt_key).order_by(PromptVersion.created_at.desc()))).scalars())


@router.post("/{prompt_key}/versions", response_model=PromptVersionSummary, status_code=201)
async def create_version(prompt_key: str, request: PromptVersionCreate, session: DatabaseSession, settings: AppSettings):
    _enabled(settings)
    try:
        record = await PromptRegistry(session).create_version(prompt_key, request.version, request.content, description=request.description, created_by=request.created_by)
...
@router.post("/{prompt_key}/versions/{version}/activate", response_model=PromptVersionSummary)
...
@router.post("/{prompt_key}/versions/{version}/archive", response_model=PromptVersionSummary)
