...
class LLMService:
        ...
        async with self.tracer.span(
            ...
            for attempt in range(self.settings.llm_max_retries + 1):
                try:
                    ...
                    span.set_metric("attempts", attempt + 1)
                    usage = getattr(response, "usage", None)
                    if usage is not None:
                        span.set_metric("input_tokens", getattr(usage, "prompt_tokens", None))
                        span.set_metric("output_tokens", getattr(usage, "completion_tokens", None))
    ...
    async def rewrite_query(self, query: str) -> dict[str, Any]:
        ...
        resolved = await self.prompt_registry.get_active_prompt("query_rewrite")
        prompt = resolved.content.format(question=query)
    ...
    async def route_analytics(self, *, question: str, tool_catalog: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        resolved = await self.prompt_registry.get_active_prompt("analytics_router")
    ...
    async def route_decision(self, *, question: str, decision_types: list[str], tool_catalog: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        resolved = await self.prompt_registry.get_active_prompt("decision_router")
        prompt = resolved.content.format(
            question=question,
