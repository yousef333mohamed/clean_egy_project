...
def report_payload(run: EvaluationRunResult, gate: QualityGateResult) -> dict:
    ...
    return sanitize({
        "run": {"id": str(run.run_id), "name": run.name, "evaluation_type": run.evaluation_type, "dataset": run.dataset_name, "dataset_version": run.dataset_version, "mode": run.mode.value},
        "prompt_versions": run.prompt_versions, "model_configuration": run.model_configuration, "metrics": run.metrics,
        "quality_gate": gate.model_dump(mode="json"), "critical_failures": run.critical_failures,
