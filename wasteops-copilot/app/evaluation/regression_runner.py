...
class RegressionRunner:
    def __init__(
        self, session: Any | None = None, settings: Settings | None = None, *, actual_provider: Any | None = None, write_report_files: bool = True
    ) -> None:
    ...
    async def run(
        self, dataset_path: Path, *, mode: EvaluationMode = EvaluationMode.FAKE_PROVIDERS, prompt_overrides: dict[str, str] | None = None
    ) -> EvaluationRunResult:
        ...
        run = EvaluationRunResult(
            name=f"{dataset.name}-{datetime.now(UTC).isoformat()}",
        ...
        if self.session is not None:
            db_run = EvaluationRun(
                ...
                model_configuration_json=run.model_configuration,
        ...
        for batch_start in range(0, len(dataset.cases), self.settings.evaluation_max_concurrency):
            outcomes = await asyncio.gather(
                *(execute(case) for case in dataset.cases[batch_start : batch_start + self.settings.evaluation_max_concurrency]), return_exceptions=True
            )
            for case, outcome in zip(dataset.cases[batch_start : batch_start + self.settings.evaluation_max_concurrency], outcomes, strict=True):
                if isinstance(outcome, BaseException):
                    outcome = EvaluationCaseResult(
                        case_id=case.case_id,
                        category=case.category,
                        passed=False,
                        score=0,
                        failure_reasons=[f"Evaluation error: {type(outcome).__name__}"],
                        duration_ms=0,
            ...
            for case, result in zip(dataset.cases, run.results, strict=False):
                self.session.add(
                    EvaluationResult(
                        ...
                        question=sanitize_text(case.question, max_length=1000),
                        expected_json=sanitize(case.model_dump(mode="json", exclude={"question", "recorded_response"})),
                        actual_json={},
    ...
    def _fake_from_expected(case):
        ...
        return {
            ...
            "supplied_source_ids": expected.get("supplied_source_ids", []),
            "answer": expected.get("answer", " ".join(case.expected_concepts)),
            "insufficient_context": case.expected_insufficient_context,
            "decision_type": expected.get("decision_type", case.expected_route),
            "action_category": (expected.get("allowed_action_categories") or [None])[0],
            "required_tools": expected.get("required_tools", []),
            "evidence_ids": expected.get("evidence_ids", ["E1"] if expected.get("requires_evidence") else []),
            "requires_human_approval": case.expected_human_approval,
            ...
            "expected_numeric_values": expected.get("numeric_values", []),
            "numeric_values": expected.get("numeric_values", []),
